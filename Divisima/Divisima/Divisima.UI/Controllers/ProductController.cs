...
namespace Divisima.UI.Controllers
{
	public class ProductController : Controller
	{
		...
		public IActionResult Detail(string name,int id)
		{
			...
			if (product != null)
			{
				//ViewBag.Title = product.Name;// layout Title ayarlayınca yap
				//return View(product);
		...
		public IActionResult getSearchProduct(string search) // Tüm ürün  ler(metodu get yapıp çalıştırınca /urun/ara?search=olo) =>
		{
			...
			{
				ProductName = x.Name,
				ProductPicture = x.ProductPictures.FirstOrDefault()
			.Picture,
				ProductLink = "/urun/" + GeneralTools.GetUrl(x.Name) + "-" + x.ID
			}));
